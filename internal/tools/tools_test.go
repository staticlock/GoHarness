...
func TestAskUserQuestionTool(t *testing.T) {
	...
	if result.Output != "yes" {
		t.Fatalf("unexpected answer output: %q", result.Output)
	}

	emptyResult, err := (&AskUserQuestionTool{}).Execute(ctx, args, ToolExecutionContext{
		CWD:      t.TempDir(),
		Metadata: map[string]any{"ask_user_prompt": func(string) string { return "   " }},
	})
	if err != nil || emptyResult.IsError {
		t.Fatalf("ask_user_question empty response failed: %v %+v", err, emptyResult)
	}
	if emptyResult.Output != "(no response)" {
		t.Fatalf("unexpected empty response output: %q", emptyResult.Output)
	}

	unavailable, err := (&AskUserQuestionTool{}).Execute(ctx, args, ToolExecutionContext{CWD: t.TempDir(), Metadata: map[string]any{}})
	if err != nil {
		t.Fatalf("unexpected execute error: %v", err)
	}
	if !unavailable.IsError || !strings.Contains(unavailable.Output, "unavailable") {
		t.Fatalf("expected unavailable error result, got %+v", unavailable)
	}
}
